...
@section('title', 'Daftar Mustahik - Sistem Zakat Fitrah')
...
<div class="py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-secondary-dark">Daftar Mustahik</h1>
                <p class="mt-2 text-secondary-DEFAULT">Kelola data mustahik (penerima zakat)</p>
            </div>
            <div>
                <a href="{{ route('mustahik.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-800 text-white font-medium rounded-lg transition duration-300 shadow-md">
        ...
        <div class="mb-6 bg-white rounded-xl shadow-custom p-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="relative flex-grow max-w-2xl">
                    ...
                    <input type="text" id="search-mustahik" class="block w-full p-3 pl-10 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Cari nama mustahik...">
                </div>
        ...
        <div class="bg-white rounded-xl shadow-custom overflow-hidden">
            @if($mustahikWarga->isEmpty())
            <div class="p-8 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                </svg>
                <p class="mt-4 text-lg font-medium text-secondary-DEFAULT">Belum ada data mustahik</p>
                <p class="text-secondary-DEFAULT">Silahkan tambahkan data mustahik baru.</p>
                <a href="{{ route('mustahik.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-800 text-white font-medium rounded-lg transition duration-300 shadow-md">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                    Tambah Mustahik
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-secondary-DEFAULT uppercase tracking-wider">Hak</th>
                            <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-secondary-DEFAULT uppercase tracking-wider">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200" id="mustahik-table-body">
                        @foreach($mustahikWarga as $index => $item)
                        <tr class="mustahik-row" data-kategori="{{ trim($item->kategori) }}" 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">{{ $item->hak }}%</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex justify-center space-x-3">
                                    <a href="{{ route('mustahik.show', $item->id_mustahikwarga) }}" class="text-blue-600 hover:text-blue-800 transition duration-300" title="Lihat Detail">
                                        <i class="fas fa-eye text-lg"></i>
                                    </a>
                                    <a href="{{ route('mustahik.edit', $item->id_mustahikwarga) }}" class="text-blue-600 hover:text-blue-800 transition duration-300" title="Edit">
                                        <i class="fas fa-edit text-lg"></i>
                                    </a>
                                    <button type="button" onclick="showDeleteModal('delete-modal-{{ $item->id_mustahikwarga }}')" class="text-red-600 hover:text-red-800 transition duration-300" title="Hapus">
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-500">
                    Menampilkan <span class="font-medium">5</span> dari <span class="font-medium">{{ $mustahikWarga->count() }}</span> data
                    <span id="kategori-info" class="hidden"> (Kategori: <span class="font-medium"></span>)</span>
                </div>
                ...
                <div class="inline-flex shadow-sm bg-white rounded-md">
                    ...
                    <div class="relative inline-flex items-center px-4 py-2 text-sm text-gray-700 bg-white border-t border-b border-gray-300">
                        ...
                        <span>/</span>
                        <span id="total-pages">{{ ceil($mustahikWarga->count() / 5) }}</span>
                    </div>
                    
                    <button type="button" id="next-page" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-r-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"{{ $mustahikWarga->count() <= 5 ? ' disabled' : '' }}>
                        Berikutnya
...
@foreach($mustahikWarga as $item)
<div id="delete-modal-{{ $item->id_mustahikwarga }}" class="fixed inset-0 z-50 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4 sm:p-0">
        ...
        <div class="relative inline-block w-full max-w-md bg-white rounded-lg shadow-xl overflow-hidden transform transition-all sm:max-w-md" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            <div class="bg-white px-6 py-6">
                <div class="text-center mb-5">
                    <p class="text-base text-gray-600 mb-2">
                        Anda akan menghapus data mustahik:
                    </p>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                Tindakan ini tidak dapat dibatalkan. Semua data terkait mustahik ini akan dihapus secara permanen dari sistem.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-gray-100">
                <form action="{{ route('mustahik.destroy', $item->id_mustahikwarga) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchInput = document.getElementById('search-mustahik');
        const rows = document.querySelectorAll('.mustahik-row');
        ...
        const totalItems = {{ $mustahikWarga->count() }};
        ...
        function showPage(page) {
            ...
            for (let i = start; i < end && i < visibleRows.length; i++) {
                ...
                // Update nomor urut
                const numCell = visibleRows[i].querySelector('td:first-child div');
        ...
        function applyFilters() {
            ...
            const selectedKategoriText = kategoriFilter ? kategoriFilter.options[kategoriFilter.selectedIndex].text : '';
            ...
            if (kategoriInfo) {
                if (selectedKategori) {
                    ...
                    const kategoriName = kategoriInfo.querySelector('span.font-medium');
                    if (kategoriName) {
                        kategoriName.textContent = selectedKategoriText;
                    }
                } else {
            ...
            rows.forEach(row => {
                ...
                const matchesKategori = selectedKategori === '' || 
                                     (kategori && kategori.toLowerCase() === selectedKategori.toLowerCase());
