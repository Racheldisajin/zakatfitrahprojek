...
<div class="py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-secondary-dark">Detail Mustahik</h1>
                <p class="mt-2 text-secondary-DEFAULT">Informasi lengkap tentang mustahik</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('mustahik.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-secondary-DEFAULT font-medium rounded-lg transition duration-300">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT">ID Mustahik</h3>
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->id_mustahikwarga }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT">Nama</h3>
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->nama }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT">NIK</h3>
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->nik }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT">Kategori</h3>
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->kategori }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->created_at->format('d F Y, H:i') }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT">Terakhir Diperbarui</h3>
                                <p class="mt-1 text-secondary-dark">{{ $mustahik->updated_at->format('d F Y, H:i') }}</p>
                            </div>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 h-full">
                            <div class="mb-4">
                                <h3 class="text-sm font-medium text-secondary-DEFAULT mb-2">Jumlah Hak</h3>
                                <p class="text-secondary-dark">{{ $mustahik->hak }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-medium text-secondary-DEFAULT mb-2">Keterangan Tambahan</h3>
                                <p class="text-secondary-dark">
                                    @if($mustahik->keterangan)
                                        {{ $mustahik->keterangan }}
                                    @else
                                        <span class="text-gray-400 italic">Tidak ada keterangan tambahan</span>
                                    @endif
                                </p>
                            </div>
                        </div>
                ...
                <div class="border-t border-gray-200 mt-8 pt-6">
                    <h2 class="text-lg font-medium text-secondary-dark mb-4">Riwayat Distribusi Zakat</h2>
                    
                    <div class="bg-gray-50 rounded-lg p-6 text-center">
                        <p class="text-secondary-DEFAULT">Data distribusi zakat akan muncul di sini setelah mustahik menerima distribusi.</p>
                        <a href="{{ route('distribusi.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-800 text-white font-medium rounded-lg transition duration-300 shadow-md">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="mt-8 flex justify-between items-center">
                    ...
                    <a href="{{ route('mustahik.edit', $mustahik->id_mustahikwarga) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-800 text-white font-medium rounded-lg transition duration-300 shadow-md">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="flex items-center justify-center min-h-screen px-4 sm:p-0">
        ...
        <div class="relative inline-block w-full max-w-md bg-white rounded-lg shadow-xl overflow-hidden transform transition-all sm:max-w-md" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-gray-100">
                <form action="{{ route('mustahik.destroy', $mustahik->id_mustahikwarga) }}" method="POST">
                    @csrf
                    @method('DELETE')
