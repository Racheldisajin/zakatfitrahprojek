...
@section('title', 'Edit Mustahik - Sistem Zakat Fitrah')
...
@section('content')
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-center">
                <div class="mb-4 md:mb-0">
                    <h1 class="text-3xl font-bold text-secondary-dark">
                        <span class="inline-block border-b-4 border-primary-DEFAULT pb-1">Edit Mustahik</span>
                    </h1>
                    <p class="mt-2 text-secondary-DEFAULT text-opacity-80">Formulir untuk memperbarui data penerima zakat dalam sistem</p>
                </div>
                <div>
            ...
            <div class="bg-white rounded-xl shadow-custom overflow-hidden border border-gray-100">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-100">
                    ...
                    <p class="text-sm text-gray-500">Perbarui data penerima zakat fitrah dengan informasi yang akurat</p>
                </div>

                <!-- Form Content -->
                <div class="p-6">
                    <form action="{{ route('mustahik.update', $mustahik->id_mustahikwarga) }}" method="POST" id="mustahik-form" class="space-y-8">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="flex flex-col h-full justify-between">
                                <div class="mb-6">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="nama" id="nama" value="{{ old('nama', $mustahik->nama) }}" 
                                ...
                                <div class="mt-auto">
                                    ...
                                    <div class="relative">
                                        ...
                                        <textarea name="alamat" id="alamat" rows="3" 
                                            class="pl-10 block w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-DEFAULT focus:border-transparent transition duration-200" 
                                            placeholder="Masukkan alamat lengkap termasuk RT/RW">{{ old('alamat', $mustahik->alamat ?? '') }}</textarea>
                                    </div>
                                    @error('alamat')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="flex flex-col h-full justify-between">
                                <div class="mb-6">
                                    ...
                                    @foreach($kategori as $kat)
                                                <option value="{{ $kat->nama_kategori }}" data-hak="{{ $kat->jumlah_hak }}" {{ (old('kategori', $mustahik->kategori) == $kat->nama_kategori) ? 'selected' : '' }}>
                                                    {{ $kat->nama_kategori }}
                                                </option>
                                    @endforeach
                                </select>
                                    </div>
                                    <p class="text-xs text-gray-500 mt-1">Kategori menentukan jumlah hak yang diterima</p>
                                    @error('kategori')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-6">
                                    <label for="hak" class="block text-sm font-medium text-secondary-DEFAULT mb-2">
                                        Jumlah Hak <span class="text-red-500">*</span>
                                    </label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                        ...
                                        <input type="number" name="hak" id="hak" value="{{ old('hak', $mustahik->hak) }}" 
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Nilai ini diisi otomatis berdasarkan kategori</p>
                                    @error('hak')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mt-auto">
                                    ...
                                    <div class="relative">
                                        ...
                                        <textarea name="keterangan" id="keterangan" rows="3" 
                                            class="pl-10 block w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-DEFAULT focus:border-transparent transition duration-200" 
                                            placeholder="Tambahkan informasi pendukung jika diperlukan">{{ old('keterangan', $mustahik->keterangan ?? '') }}</textarea>
                                    </div>
                                    @error('keterangan')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="pt-6 border-t border-gray-200 flex flex-col sm:flex-row-reverse gap-3">
                            <button type="button" onclick="showUpdateModal()" class="w-full sm:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition duration-300 shadow-md flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                </svg>
                                Perbarui Data Mustahik
                            </button>
                            <a href="{{ route('mustahik.index') }}" class="w-full sm:w-auto px-6 py-3 bg-gray-100 hover:bg-gray-200 text-secondary-DEFAULT font-medium rounded-lg transition duration-300 flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div id="update-modal" class="fixed inset-0 z-50 overflow-y-auto hidden">
                ...
                <div class="bg-white px-6 py-6">
                    <div class="text-center mb-5">
                        <p class="text-base text-gray-600 mb-2">
                            Anda akan memperbarui data mustahik:
                        </p>
...
@endsection
    ...
    <script>
        ...
        // Kategori change event and modal setup
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('confirm-update-button').addEventListener('click', function() {
                try {
                    ...
                    // Verifikasi CSRF token dan method PUT
                    ...
                    if (!methodField) {
                        console.error('Method PUT tidak ditemukan!');
                        // Tambahkan method field jika tidak ada
